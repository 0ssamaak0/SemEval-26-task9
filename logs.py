...
def log_dspy(
    ...
    """
    Simpler logging function for DSPy experiments.
    Logs subtask_num and evalresult.
    Creates a new trial with trial_id if provided, otherwise generates a new one.
    """
    subtask_dict = {f"subtask_{subtask_num}": {"evalresult": evalresult}}
            ...
            if trial_id is None:
                ...
                for trial in logs:
                    if trial.get("trial_id") == trial_id:
                        ...
                        trial[f"subtask_{subtask_num}"] = {"evalresult": evalresult}
                        if metadata is not None:
                            trial["metadata"] = metadata
                        found = True
                        break
